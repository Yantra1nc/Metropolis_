 ...
 * 1.00       31 Jul 2020     Ganesh Sapkale	MHL- SCH- YIL Bank Deposit re-process.js
 *
 */
...
function scheduled(type) {
    try {
        ...
        var searchId = context.getSetting('SCRIPT', 'custscript_mhl_sch_bank_dep_');
        ...
        if (_logValidation(jsonArray)) {
            ...
            //var sch_script = nlapiScheduleScript('customscript_bank_deposit_json_processor', 'customdeploy_bank_deposit_json_processor', null);
            //End
            // Code added by Kailas on 30th nov 2020
            //if (deplymentId != 'customdeploy5')

            var sch_script = nlapiScheduleScript('customscript_mhl_sch_bank_dep_re_push', deplymentId, null);
            //End
        ...
        var sch_script = nlapiScheduleScript('customscript_bank_deposit_json_processor', deplymentId, null);
        // End
        nlapiLogExecution('DEBUG', 'sch_script', sch_script);
    }
}
    ...
    try {
        ...
        if (s_locationCode) {
            ...
            if (locSearch) {
                ...
                var orgIdTxt = locSearch[0].getText('custrecord_mhl_loc_org');
                ...
                if (searchAccount) {
                    ...
                    if (advance) {
						...
						// Setting Inward Transactions
						for (var t in JSONObj.requestObject.inwardTransactions) {
							
							if (JSONObj.requestObject.inwardTransactions[t].clientCode == "GENERAL") {
								var clientCodeOrg = JSONObj.requestObject.inwardTransactions[t].clientCode + JSONObj.requestObject.inwardTransactions[t].orgId;
							} else {
								var clientCodeOrg = JSONObj.requestObject.inwardTransactions[t].clientCode;
							}
							
							var filter_cust = [];
							var cols_cust = [];
							filter_cust.push(new nlobjSearchFilter('entityid', null, 'is', clientCodeOrg).setOr(true));
							...
							//nlapiLogExecution("debug","customerClientSearch",JSON.stringify(customerClientSearch))
							...
							VIDRefRec.setFieldValue('custrecord_mlh_cms_vid_amount', JSONObj.requestObject.inwardTransactions[t].amount);
                   ...
                   /*  var filter = [];
                    ...
                    if (vids) {
                        ...
                        for (var t in JSONObj.requestObject.vidTansactions) {
							...
							if(JSONObj.requestObject.vidTansactions[t].vid)
							{
								...
								vidNotFoundAmt = Number(vidNotFoundAmt) + Number(JSONObj.requestObject.vidTansactions[t].amount);
								var VIDRefRec = nlapiCreateRecord('customrecord_mhl_deposited_vid_child');
								VIDRefRec.setFieldValue('custrecord_mhl_deposit_vid', JSONObj.requestObject.vidTansactions[t].vid);                         
								VIDRefRec.setFieldValue('custrecord_mlh_cms_vid_amount', JSONObj.requestObject.vidTansactions[t].amount);
								if (JSONObj.requestObject.vidTansactions[t].clientCode == "GENERAL") {
									var clientCodeOrg = JSONObj.requestObject.vidTansactions[t].clientCode + JSONObj.requestObject.vidTansactions[t].orgId;
								} else {
									var clientCodeOrg = JSONObj.requestObject.vidTansactions[t].clientCode;
								}
                    ...
                    if (depositRecord.getLineItemCount('other') > 0) {
						...
						depositRecord.setFieldValue('custbody_mhl_vid_ref_rec', VIDRefParentRecId)
						var depositId = nlapiSubmitRecord(depositRecord, true, true);
						nlapiLogExecution('Audit', 'depositId', depositId);
						nlapiSubmitField('customrecord_mhl_deposited_vids_ref', VIDRefParentRecId, 'custrecord_mhl_deposit_id', depositId)
                ...
                // If account not found error
					...
					createRnIRecord('Account not found in Payment Account Mapping record for Bankid '+ bankId +" of Org "+orgIdTxt,fileObj, recieptNo, locationId, bankId)
