 ...
 * Script Name: MR YIL RnI VID separation JSON
 * File Name: MHL YIL JSON Separation file based on cust payment mode.js
 * Created On: 23/05/2023
 * Modified On:
 * Created By: Avinash Lahane(Yantra Inc.)
 * Modified By:
 * Description:  RnI VID separation JSON
 *********************************************************** */
	    ...
	    function (file, format, record, search, runtime, mhllib, datellib) {
	        ...
	        function getInputData() {
	            ...
	            try {
	                ...
	                return search.load({
	                    id: 'customsearch_vid_separation'
	                });
	        ...
	        function map(context) {
	            try {
	                ...
	                log.audit('File Moved | '+fileInternalId, invId );
	                ...
	                if (invId == '-B2C') {
	                 ...
	                 jsonFile.save();
	                    ...
	                    jsonFile.folder = '566111';
	                    jsonFile.save();
					} else if( invId == 'Location Not found'){
	                    jsonFile.folder = '337799';
	                    jsonFile.save();
	                }else {
	                    jsonFile.folder = '565889';
	                    jsonFile.save();
	                }
	                ...
	                createRnIRecord(ex, jsonFile.name,'','',"File separation(RnI- B2C/B2B) script error");
	                jsonFile.folder = '707';
	                jsonFile.save();
	            }
	        }
	        ...
	        function createVIDForJson(JSONfromRnI, fileName, fileId) {
	            try {
					...
					{
						log.error("Client Not found",JSONfromRnI.VisitInfo.VisitNumber+" Org Name "+orgName)
						createRnIRecord('Client code Not Found | ' + JSONfromRnI.VisitInfo.ClientCode +" for Org "+orgName+" | VID | " + JSONfromRnI.VisitInfo.VisitNumber, fileName, fileId,'',"Client code Not Found");
