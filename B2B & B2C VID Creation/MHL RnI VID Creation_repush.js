/**
 * Script Name: MHL RnI VID Creation _Optimized.js
 * @NApiVersion 2.x
 ...
 * Script Name: MHL RnI VID Creation _Process
 * File Name:  MHL RnI VID Creation _Optimized.js
 * Created On: 02/02/2022
 * Modified On:
 * Created By: Ganesh Sapakale(Yantra Inc.)
 * Modified By:
 * Description:  RnI VID Creation _Process
 *********************************************************** */
    ...
    function(file, format, record, search, runtime, mhllib, datellib) {
        ...
        function getInputData() {
            ...
            try {
                ...
                if (deploymentId == 'customdeploy_mhl_rni_vid_creation_repush') {
                    return search.load({
                        id: 'customsearch_vid_for_repush_manual'
                    });
                }

                if (deploymentId == 'customdeploy_mhl_rni_vid_creation_reproc') {
                    return search.load({
                        id: 'customsearch_mhl_vid_optimised'
                    });
                }
        ...
        function map(context) {
            try {
                ...
                if (invId > 0) {
                    ...
                    jsonFile.folder = '4343';
                    jsonFile.save();
                } else if (invId == '-356151') {
                    jsonFile.folder = '356151';
                    jsonFile.save();
                } else if (invId == '-341975') {
        ...
        function createVIDForJson(JSONfromRnI, fileName, fileId) {
            try {
                ...
                if (visitDate) {
                    ...
                    if (visitNumberToBeFound)
                        invoiceRecId = searchInvoice(visitNumberToBeFound);
                    ...
                    if (!invoiceRecId) {
						...
						*/
                        if (orgInternalId) {
                            ...
                            if (customerDetails) {
                                ...
                                if (customerSegment && customerSegment != '' && customerSegment != ' ') {
                                    ...
                                    if (consolidatedItem) 
									{
                                        ...
                                        //Timestamp case: 
                                        var i_subsidiaryId = invoiceRecord.getValue({
                                            fieldId: 'subsidiary'
                                        });

                                        log.debug("i_subsidiaryId", i_subsidiaryId);
                                        ...
                                        invoiceRecord.setValue({
                                            fieldId: 'custbody_mhl_vid_optimization',
                                            value: true
                                        });
                                        invoiceRecord.setValue({
                                            fieldId: 'location',
                                        ...
                                        // This code added By Ganesh (Yantra Inc)
                                        log.debug("External Id", "inv_" + JSONfromRnI.VisitInfo.VisitNumber)
                                        invoiceRecord.setValue({
                                            fieldId: 'externalid',
	                                    ...
	                                    if (gbiLength.length > 0) {
                                            ...
                                            for (var te in testArray) {
                                                ...
                                                if (testArray[te].ProcessingLocationCode) {
                                                    ...
                                                    createRnIRecord("Processing Location Code not found for " + JSONfromRnI.VisitInfo.VisitNumber, fileName, fileId,'Processing Location Code not found');
                                                    return '-356151';
                                                }
                                        ...
                                        for (var pay in paymentInfo) {
											...
											{												
												...
												{
													var paymentId = paymentRecordFunction(trans_date, subsidiary, invoiceId, customerId, Number(paymentInfo[pay].AmountReceived), vidDate, paymentInfo[pay], payMode, vidNumber, orgVal, departmentVal, classmentVal, unitVal);
													// this code added by Ganesh
													if (paymentId != 'payment_created') {
														createRnIRecord('Customer Payment Not Created ' + paymentId + ' \n For VID ' + JSONfromRnI.VisitInfo.VisitNumber, fileName, fileId, invoiceId,"Payment Not Created");
														return '-341975';
													}
												}
                                        ...
                                        createRnIRecord('Revenue Item not found for segment ' + customerSegment + ' For VID ' + JSONfromRnI.VisitInfo.VisitNumber, fileName, fileId,'',"Revenue Item not found");
                                    ...
                                    createRnIRecord('Revenue Segment Missing for Client Code ' + JSONfromRnI.VisitInfo.ClientCode + ' & Registered ORD Internal ID ' + registeredOrg + ' & VID ' + JSONfromRnI.VisitInfo.VisitNumber, fileName, fileId,'',"Revenue Segment Missing for Client Code");
                                    return '';
                                }
                            ...
                            createRnIRecord('Registered Location details not found ' + JSONfromRnI.VisitInfo.locationCode + ' For VID ' + JSONfromRnI.VisitInfo.VisitNumber, fileName, fileId,'',"Registered Location details not found");
                            return '-337799';

                        }
                    } else {
                        //log.debug('Duplicate VID number Found','VID number already present in NetSuite');
                      log.error("Duplicate VID Found", JSONfromRnI.VisitInfo.VisitNumber)
                        createRnIRecord('Duplicate VID Number found ' + JSONfromRnI.VisitInfo.VisitNumber, fileName, fileId,'', "Duplicate VID Number found");
                        return '-234';
                    }
                }
				else{
					createRnIRecord('Visit Date is not available in JSON for ' + JSONfromRnI.VisitInfo.VisitNumber, fileName, fileId,'', "Visit Date is not available in JSON");
					return '';
				}
                ...
                createRnIRecord('Error Occured for For VID ' + JSONfromRnI.VisitInfo.VisitNumber + ' details ' + JSON.stringify(inve), fileName, fileId,'', "Suitescript Errors");
            }
        }
        ...
        /////////////////////////////////////////////// Create Error Record ////////////////////////////////////////////////////////
        function createRnIRecord(e, fileName, fileId, invoiceId,errorTitle) {
                ...
                if ((customerType.custentity_mhl_customer_payment_mode[0].text == 'Cash' || customerType.custentity_mhl_customer_payment_mode[0].text == 'Co-payment') && amountReceived > 0 && tranRefNo) {
					...
					customrecord_mhl_payment_mode_mappingSearchObj.run().each(function(result){
					   ...
					   var i_paymemtMode = result.getValue({name: "custrecord_netsuite_payment_mode_id"});
						...
						if (tranRefNo.Cheque_Card_DDNo) {
                            paymentRecord.setValue({
                                fieldId: 'custbody_payment_transaction',
                                value: tranRefNo.Cheque_Card_DDNo
                            });
