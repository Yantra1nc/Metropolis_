	...
	define(['N/file', 'N/format', 'N/record', 'N/search', 'N/runtime', './mhllib', './datellib', './searchlib', 'N/url', 'N/email'],
	    ...
	    function(file, format, record, search, runtime, mhllib, datellib, searchlib, url, email) {
	            ...
	            try {
	                ...
	                var customrecord_b2b_vid_detailsSearchObj = search.create({
	                    ...
	                    filters: [
	                        ...
	                        "AND",
	                        ["custrecord_mhl_tran_main_vid.mainline", "is", "T"]
							,"AND",
	                        //["internalidnumber", "equalto", "1363072"]//SB
	                        ["internalidnumber", "equalto", "17677802"]	//Prod
	                        //["internalidnumber", "equalto", "17627309"]	//Prod
	                        //["internalidnumber", "equalto", "18070909"]	//Prod
	                        //["internalid", "anyof","1363072","1363916"] //SB							
	                    ],
	                    columns: [
	                        search.createColumn({
	                ...
	                if (resultSet) {
	                    for (var i = 0; i < resultSet.length; i++) {
	                        ...
	                        var b2cInvInternalId = resultSet[i].getValue({
	                            name: "internalid",
	                            join: "CUSTRECORD_MHL_TRAN_MAIN_VID",
	                            label: "Internal ID"
	                        });
	                        log.debug("b2cInvInternalId ---->", b2cInvInternalId);

	                        var sisTestCode = resultSet[i].getValue({
	                            name: "custrecord_test_code_json",
								join: "CUSTRECORD_REFERENCE_B2B",
								label: "Test Code String"
	                        });
	                        log.debug("sisTestCode ---->", sisTestCode);
	                        ...
	                        log.debug("vid_number ---->", vid_number);
	                        ...
	                        log.debug("b2b_Vid_Org ---->", b2b_Vid_Org);
	                        ...
	                        log.debug("client_name ---->", client_name);
	                        ...
	                        log.debug("b2b_Parent_Amount ---->", b2b_Parent_Amount);

	                        var b2b_Sister_LabVid = resultSet[i].getValue({
	                            name: "custrecord_vidno",
	                            label: "VID No"
	                        });
	                        log.debug("b2b_Sister_LabVid ---->", b2b_Sister_LabVid);
	                    ...
	                    if (b2bVidInternalId_reduce) {
	                        var customrecord_b2b_vid_second_search = search.create({
	                            type: "invoice",
                                    filters: [
                                        ["type", "anyof", "CustInvc"],
                                        "AND",
                                        ["internalid", "anyof", b2bVidInternalId_reduce],
                                        "AND",
                                        ["mainline", "is", "T"],
                                        "AND",
                                        //["custrecord_mhl_itd_vid.custrecord_mhl_itd_test_code", "anyof", b2bSisTestCode_reduce],
                                        ["custrecord_mhl_itd_vid.custrecord_mhl_itd_test_code_string","is",b2bSisTestCode_reduce],
										"AND", 
										["custrecord_mhl_itd_vid.custrecord_mhl_itd_net_amt","isnotempty",""]
                                    ],
                                    columns: [
                                        search.createColumn({
                                            name: "custbody_mhl_invoice_vid_number",
	                        ...
	                        if (resultSetTwo.length > 0) {
	                            for (var j = 0; j < resultSetTwo.length; j++) {
									...
									var forreignAmt = resultSetTwo[j].getValue({name: "fxamount", label: "Amount (Foreign Currency)"});
									log.debug("forreignAmt ---->", forreignAmt);
										
									/* if(forreignAmt){
										b_vidTestAmount = forreignAmt;
									} */
									...
									/* else if(b_vidNumberNew == vidNumber_reduce){
										inHouseRevenue = b_vidNetAmount - b_vidNetAmount;
									}else{
										log.debug("");
									}
	        ...
	        function summarize(summary) {
	            ...
	            //newXLS_File.folder = 502667; //SB
	            newXLS_File.folder = 1270916;
				...
				try{
				 ...
				 //var PDFPrint;
                    ...
                    PDFPrint += "<br>";
                    PDFPrint += "B2C Inhouse Revenue Report.<br>";
					PDFPrint += "<br>";
