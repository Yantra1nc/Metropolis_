 ...
 * @NModuleScope SameAccount
	...
	* Date: May 2022
	* Description: This script will Add test code on RNI VID record.
 */
define(['N/file', 'N/format', 'N/record', 'N/search', 'N/runtime', './mhllib'],
    ...
    function(file, format, record, search, runtime, mhllib) {
        ...
        function getInputData() {
            ...
            try {
                return search.load({
                    id: 'customsearch_mhl_b2b_rni_test_addtion'
                });
            ...
            try {
                ...
                if (invoiceRecId) {
                        ...
                        var customerDetails = searchCustomer(JSONfromRnI.VisitInfo.ClientCode, registeredOrg, custSubsidiary, JSONfromRnI.OrgID);
                            ...
                            if (customerSegment && customerSegment != '' && customerSegment != ' ') {
                                ...
                                if (consolidatedItem) {
                                    ...
                                    var s_subsidiary = invoiceRecord.getText({
                                        fieldId: 'subsidiary'
                                    });
                    ...
                    if (i_B2B_VID) {
                        ...
                        var customerID = o_vidObj.getValue({
                            fieldId: "custrecord_clientname"
                        });
                        log.debug("customerID", customerID);
                        var soStatus = o_vidObj.getValue({
                            fieldId: "custrecord_mhl_b2b_so_status"
                        });
                        log.debug("soStatus", soStatus);
                        var name = o_vidObj.getValue({
                            fieldId: "name"
                        });
                        log.debug("name", name);
                        var vidNo = o_vidObj.getValue({
                        ...
                        var vidSbu = o_vidObj.getValue({
                            fieldId: "custrecord_mhl_vid_sbu"
                        });
                        log.debug("vidSbu", vidSbu);
                        var vidOrg = o_vidObj.getValue({
        ...
        function search_cluster_price(itemInternalId, registeredOrg) {
            ...
            if (searchResult.length > 0) {
                log.debug({
                    title: 'cluster rec id',
                    details: searchResult[0].id
                });
