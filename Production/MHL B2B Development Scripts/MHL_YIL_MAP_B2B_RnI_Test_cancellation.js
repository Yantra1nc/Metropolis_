/**
 * Script Name: MHL_YIL_MAP_B2B_RnI_Test_cancellation.js
 * @NApiVersion 2.x
 * @NScriptType MapReduceScript
 * @NModuleScope SameAccount
   	* Author:Ganesh Sapakale & Avinash Lahane
	* Date: Nov 2022
	* Description: This script will Update the cancellation flag on RNI VID test code details record.
 */
define(['N/search', 'N/record', 'N/runtime', 'N/file','./datellib'],
    ...
    function(search, record, runtime, file,datellib) {
        ...
        function getInputData() {
            ...
            try {
                return search.load({
                    id: 'customsearch_mhl_b2b_rni_test_cancel'
                });
            } catch (e) {
                log.debug({
        ...
        function cancelAndRefund(content, jsonFile, tranDate) {
            try {
				...
				var result =  CancellationDate.indexOf('-');
				if(result > 0){
					log.debug('New Rni Test cancel file', vidNumber);
					jsonFile.folder = '871228';
					jsonFile.save();
					return false;
				}
				else{
				log.debug("Old Rni Cancellation",CancellationDate);
				}
                        ...
                        log.debug("Org", Org);
                            ...
                            if (SoID) {
								...
								log.debug("cancellationDate", CancellationDate);
									...
									createRnIRecord('CancellationDate Not found in JSON' + vidNumber, jsonFile.name, jsonFile.id);
								}
                             ...
                             /*    var creditMemo = record.transform({
                                ...
                                n_TestCode = tempArray[te].TestCode;
									...
									createRnIRecord('Credit Memo is already created ' + vidNumber, jsonFile.name, jsonFile.id);
									}
                            ...
                            for (var i in tempArray) {
                                if (i == 0) {
                                    ...
                                    if (valTestCode) {
										...
										createRnIRecord(tempArray[i].TestCode + ' Test code which is already cancelled in the NetSuite ' + vidNumber, jsonFile.name, jsonFile.id);
                                        return false;
                                    }
                                } else {
                                    formed_testcode_string += tempArray[i].TestCode;
				...
				//B2C
                    ...
                    if (invId) {
                        ...
                        //if (custType == 1) // B2C
                            ...
                            for (var te in tempArray) {
							if(tempArray[te].NSNetAmount){
								n_totalAmount = n_totalAmount + Number(tempArray[te].NSNetAmount);
							}
							else{
                                n_totalAmount = n_totalAmount + Number(tempArray[te].NetAmount);
							}
							...
							for (var te in tempArray) {
                                ...
                                jsonFile.folder = '696';
                                jsonFile.save();
                                createRnIRecord('Credit Memo Not Created for ', jsonFile.name, jsonFile.id);
                            }
                    ...
                    if (_validateData(searchResultCount)) {
                        ...
                        createRnIRecord('Json file is found in' + fileFolder + ' | ' + vidNumber, jsonFile.name, jsonFile.id);

                        //log.debug("error record created..");
                    } else {
                        createRnIRecord('json file not found' + vidNumber, jsonFile.name, jsonFile.id);
                        jsonFile.folder = '696';
                        jsonFile.save();
                    }
                }
