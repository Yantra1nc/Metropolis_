 ...
 * Script Name: MHL_SUT_SOA Report Result
 * File Name: MHL_SUT_SOA Report Result.js
 * Created On: 20/12/2021
 * Modified On:
 * Created By: (Yantra Inc.)
 * Modified By:
 * Description: SOA Report Result
 *********************************************************** */
 ...
 */
define(['N/ui/serverWidget','N/runtime','N/record','N/redirect','N/search','N/xml','N/config','N/render','N/format','N/file','N/url','N/email'],

function(ui,runtime,record,redirect,search,xml,config,render,format,file,url,email) {
		...
		if(request.method == 'GET')
    	{
    		...
    		//	var enddate = format.format({value:d_to_date,type:format.Type.DATE});
    			...
    			var d_today =  formatDate(today);    			
    		  	...
    		  	var o_customer =record.load({type: 'customer',id: i_customerId, isDynamic: true});
				var s_customer_name = o_customer.getValue({fieldId : 'entityid'});
				var s_company_name = o_customer.getValue({fieldId : 'companyname'});
				if(!s_company_name)
					s_company_name = '';
				var s_customer_address = o_customer.getValue({fieldId : 'defaultaddress'});
				var s_customer_subsidiary = o_customer.getText({fieldId : 'subsidiary'});
				var i_customer_subsidiary = o_customer.getValue({fieldId : 'subsidiary'});
				var custCreatedDate = o_customer.getValue({fieldId : 'datecreated'});
				...
				var o_subsidiary =record.load({type: 'subsidiary',id: i_customer_subsidiary, isDynamic: true});

				var s_currency = o_subsidiary.getText({fieldId : 'currency'});
				var i_logo_id = o_subsidiary.getValue({fieldId : 'pagelogo'});
				var s_customer_subsidiary = o_subsidiary.getValue({fieldId : 'legalname'});
				
				log.debug("i_logo_id",i_logo_id)
				if(i_logo_id)
				{
					var fileObj = file.load({id: i_logo_id});
				...
				if(download_fileType == 'pdf')
				{
	        		...
	        		xmlBody += "<body font-size=\"10\" header=\"smallheader\" header-height=\"37mm\">";
	      			...
	      			xmlBody += "	<\/tr>";
	      			xmlBody += "	<tr>";
	      			xmlBody += "		<td align=\"left\" width=\"30\"  style=\"font-size: 11pt;\"><b>"+ xml.escape({xmlText :s_customer_name})+"</b><br />"+xml.escape({xmlText :s_customer_address})+"<\/td>";// style=\"font-size: 12pt;\"
	    			...
	    			xmlBody += "<td border-width=\"0.1\" width=\"15%\" align=\"justified\" colspan=\"1\" ><b>"+ xml.escape({xmlText :description})+"</b></td>";
	    			...
	    			var search_cs = search.load({ id: 'customsearch_soa_detailed_data_3'});
	    	    	//if(search_customerStatement)
	    			{
	    	    		//Add Filter to search 
	    	        	search_cs.filters.push(search.createFilter({ name: 'name', operator: search.Operator.ANYOF, values: i_customerId}));
	    	        	search_cs.filters.push(search.createFilter({ name: 'trandate', operator: search.Operator.ONORAFTER, values: startdate}));
	    	        	search_cs.filters.push(search.createFilter({ name: 'trandate', operator: search.Operator.ONORBEFORE, values: enddate}));
	    	        	
	    	        	var resultSet = search_cs.run().getRange({start: 0,end: 1000});
	    	                   ...
	    	                   /*  if(i_credit)
	    	                    ...
	    	                    //if(i_debit && i_credit)
	    	                    {
	    	                    	if(i_credit)
	    	                    	{
	    	    	        			...
	    	    	        			xmlBody += "<td border-width=\"0.1\" width=\"13%\" align=\"right\" colspan=\"1\" >"+ xml.escape({xmlText :i_credit})+"</td>";
	    	    	        			xmlBody += "<td border-width=\"0.1\" width=\"13%\" align=\"right\" colspan=\"1\" >"+ xml.escape({xmlText :blank})+"</td>";
	    	                    	...
	    	                    	if(i_debit)
	    	                    	{
	    	    	        			...
	    	    	        			xmlBody += "<td border-width=\"0.1\" width=\"15%\" align=\"justified\" colspan=\"1\" >"+ xml.escape({xmlText :s_description})+"</td>";
	    	    	        			xmlBody += "<td border-width=\"0.1\" width=\"13%\" align=\"right\" colspan=\"1\" >"+ xml.escape({xmlText :blank})+"</td>";
	    	    	        			xmlBody += "<td border-width=\"0.1\" width=\"13%\" align=\"right\" colspan=\"1\" >"+ xml.escape({xmlText :i_debit})+"</td>";
	    	    	        			xmlBody += "<td border-width=\"0.1\" width=\"14%\" align=\"right\" colspan=\"1\" >"+ i_total_amount+"</td>";
	    	    	        			xmlBody += "</tr>";
	    	                    	}
	    	                    }
						...
						xmlBody += "<td border-width=\"0.1\" width=\"15%\" align=\"justified\" colspan=\"1\" ><b>"+ xml.escape({xmlText :endDescription})+"</b></td>";
	        			xmlBody += "<td border-width=\"0.1\" width=\"13%\" align=\"center\" colspan=\"1\" >"+ xml.escape({xmlText :blank})+"</td>";
	        			xmlBody += "<td border-width=\"0.1\" width=\"13%\" align=\"center\" colspan=\"1\" >"+  xml.escape({xmlText :blank})+"</td>";
					...
					if(i_soa_file_Id){
                        ...
                        {
                            log.error("Email Sending error on ",e.message);
                        }
					} 
				...
				else if(download_fileType == 'csv')
				{
    	        	...
    	        	var fileObj = file.create({name : s_req_soa_file_name,fileType: file.Type.CSV,contents: s_soa_Contents});
    		...
    		{
    			log.error('Catch','msg: '+error.name +'#'+error);

    			if(error.name == 'SSS_SEARCH_TIMEOUT' || error.name == 'SSS_MISSING_REQD_ARGUMENT')
    			{
    				var pageNotFound = ui.createForm({ title: 'We are facing server issue. Please retry again.' });
					response.writePage(pageNotFound);
    			}
    			else
    			{
    				var pageNotFound = ui.createForm({ title: 'We encountered below error - '+error});
					response.writePage(pageNotFound);
    			}    			
    		}    		
    ...
    function formatDate(today)
    {
    	...
    	var responseDate=format.parse({value:today,type:format.Type.DATE});
    	var finalresponseDate=format.format({value:responseDate,type:format.Type.DATE});
    ...
    function getOpeningBalance(i_customerId, startdate)////custCreatedDate
    {
    	...
    	var search_vs = search.load({ id: 'customsearch_soa_opening_balance'});
    	//if(search_vendorStatement)
		{
    		//Add Filter to search 
        	var filter = new Array();
        	var filter1 = search.createFilter({ name: 'name', operator: search.Operator.ANYOF, values: i_customerId});
        	search_vs.filters.push(filter1);
        	var filter2 = search.createFilter({ name: 'trandate', operator: search.Operator.BEFORE, values: startdate});//
        	...
        	var search_customerStatement= search_vs.run().getRange({start: 0,end: 1});
        	...
        	if(search_customerStatement != null || search_customerStatement != '')
        	{
				...
				for (var counter = 0; counter < length; counter++)
				{
					...
					amount = result[counter].getValue({name: "fxamount",summary: "SUM"});
					if(amount)
					{
