...
define(['N/file', 'N/format', 'N/record', 'N/search', 'N/runtime', './mhllib','N/task'],
    ...
    function (file, format, record, search, runtime, mhllib,task) {
               ...
               log.debug('i_process_SBU', i_process_SBU);
				...
				if(i_customer_Id)
				{
					...
					else
					{
                        ...
                        log.debug("interIntra ",interIntra);
                        log.debug("i_customer_Id ", i_customer_Id);
						...
						if(customerDetails)
						{
							...
							if(vendorId) 
							{
								...
								var id = record.submitFields({
									...
									id: testWiseInvRecId,
									values: {
									   custrecord_mhl_b2b_cost_error : 'Vendor Id not found for Customer | '+i_customer_Text +" | Register Org | "+registerOrgText+" | Parent Org "+processingOrgText+" | for interIntra | "+interIntra	
									},
									options: {
										enableSourcing: true,
										ignoreMandatoryFields: true
							...
							var id = record.submitFields({
								...
								id: testWiseInvRecId,
								values: {
								   custrecord_mhl_b2b_cost_error : 'Vendor mapping not found for Customer | '+i_customer_Text +" | Register Org | "+registerOrgText+" | Parent Org "+processingOrgText+" | for interIntra | "+interIntra	
								},
								options: {
									enableSourcing: true,
									ignoreMandatoryFields: true
        ...
        function searchCustomer(registerLoc, processingLoc, interIntra, i_customer_Id) {
            ...
            log.debug(" registerLoc ", registerLoc);
