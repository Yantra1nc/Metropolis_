/**
 * Script Name: MHL MR SisterLab VID Creation.js
 * @NApiVersion 2.x
 ...
 * Script Name:Sisterlab VID Creation
 * File Name: MHL MR SisterLab VID Creation (16).js
 * Created On: 23/05/2023
 * Modified On:
 * Created By: Ganesh Sapakale(Yantra Inc.)
 * Modified By:
 * Description: Sisterlab VID Creation
 *********************************************************** */
...
define(['N/file', 'N/format', 'N/record', 'N/search', 'N/runtime', './mhllib','./datellib'],
  ...
  function getInputData() {
    ...
    try {
      ...
      if (deploymentId == 'customdeploy_map_mhl_sisterlab_vid') {
       /*  return search.load({
          id: 'customsearch_vid_for_sister_lab'
        });  */ 
		return search.load({
          id: 'customsearch_vid_for_sister_lab_3'
        });
      }

      if (deploymentId == 'customdeploy_mhl_sislab_1509') {
        return search.load({
  ...
  function map(context) {
    try {
      ...
      if (invId > 0) {
        jsonFile.folder = '168926'; //Sister Lab VID Processed
        jsonFile.save();
      } else if (invId == '-234') {
        jsonFile.folder = '168929'; //Sister Lab VID Duplicate
        jsonFile.save();
      } else if (invId == '-235') {
        jsonFile.folder = '168930'; //Sister Lab Test Code missing in VID
        jsonFile.save();
      } else if (invId == '-237') {
        jsonFile.folder = '168928'; //Sister Lab Main VID not Found
        jsonFile.save();
      } else {
        jsonFile.folder = '168927'; //Sister Lab VID Error
        jsonFile.save();
      }
    } catch (ex) {
      log.error({
  ...
  function createVIDForJson(JSONfromRnI, fileName, fileId) {
    try {
			...
			if (s_locationCode) {
				...
				if (searchResultCount > 0) {
					customrecord_cseg_mhl_locationsSearchObj.run().each(function(result) {
						...
						i_org_Id = parseInt(result.getValue({
							name: "custrecord_mhl_org_id"
						}))
            ...
            if (customerDetails) {
              ...
              if (customerSegment && customerSegment != '' && customerSegment != ' ') {
                ...
                if (consolidatedItem) {
                  ...
                  invoiceRecord.setValue({
                    fieldId: 'externalid',
                    value: 'inv_' + s_VID_number + i_parentLocationID + s_sis_loc_id
                  });
                ...
                createRnIRecord('Revenue Segment Missing for Client Code ' + JSONfromRnI.VisitInfo.ClientCode + ' & Registered Org Internal ID ' + registeredOrg + ' & VID ' + JSONfromRnI.VisitInfo.VisitNumber, fileName, fileId);
                return '';
              }

            } else {
              //log.debug('Can Not Create VID','Customer Not Found');
              createRnIRecord('Client code Not Found ' + JSONfromRnI.VisitInfo.ClientCode + ' For Registered ORG ID ' + i_org_Id + ' & For VID ' + JSONfromRnI.VisitInfo.VisitNumber, fileName, fileId);
      ...
      createRnIRecord('Error Occured for For VID ' + JSONfromRnI.VisitInfo.VisitNumber + ' details ' + JSON.stringify(inve), fileName, fileId);
    }
  }

  function searchInvoice(vid, s_sis_loc_id) {
    ...
    var vidSearch = search.create({
      ...
      filters: [
        ['custbody_mhl_invoice_vid_number', 'is', vid], 'AND', ['custbody_mhl_sisterlab_loc_id', 'is', s_sis_loc_id], 'AND', ['mainline', 'is', 'T']
      ]
    });
  ...
  function updateInternalId(testObject, a_item_line_data) {
    ...
    //log.audit({title:'Item ID',details:testObject.testCode });
    //log.audit({title:'Item ID',details:JSON.stringify(testObject )});
    ...
    log.debug("updateInternalId | testArray length ", testArray.length)
  ...
  function getCompanyCurrentDateTime(number) {
    ...
    var currentDateTime = new Date(Number(number));
    log.debug({
      title: 'currentDateTime',
      details: new Date(currentDateTime)
    });
    ...
    log.debug({
      title: 'companyDateTime',
      details: new Date(companyDateTime)
    });
