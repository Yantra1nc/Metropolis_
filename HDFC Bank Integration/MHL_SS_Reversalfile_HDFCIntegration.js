 ...
 * Script Name: MHL_SS_Reversalfile_HDFCIntegration
 * File Name: MHL_SS_Reversalfile_HDFCIntegration.js
...
function(record, search, runtime, email,format,file,task,sftp,encode) {
          ...
          log.debug('fileList.length',fileList.length);
			...
			for(var i=0;i<fileList.length;i++)
			{
				...
				var fileSearchObj = search.create({
				   ...
				   filters:
				   [
					  ["folder","anyof","772523","772742"], 
					  "AND", 
					  ["name","is","Reverse_"+f_name+".txt"]
				   ],
				   columns:
				   [
					  search.createColumn({
				...
				var fileObj = file.create({name: 'Reverse_'+f_name+".txt", fileType: file.Type.PLAINTEXT, contents: hexEncodedString});//.getContents()
				fileObj.folder = 772523;
	...
	function get_global_parameters()
	{
		...
		a_filters_GP.push(search.createFilter({name: 'name',operator: search.Operator.IS,values :"SFTP Details For Reverse File"}));					
		...
		var a_search_results_GP_OBJ = search.create({type: 'customrecord_mhl_netsuite_qlik_config',filters: ["name","is","SFTP Details For Reverse File"],columns: a_columns_GP});
