/**
 * Script Name: MHL_YIL_MAP_B2B_Attune_Test_cancellation.js
 * @NApiVersion 2.x
 ...
 * Script Name: MHL YIL MAP B2B Attune Test Cancellation
 * File Name: MHL_YIL_MAP_B2B_Attune_Test_cancellation.js
 * Created On: 29/05/2022
 * Modified On:
 * Created By: Avinash Lahane(Yantra Inc.)
 * Modified By:
 * Description: B2B Attune Test Cancellation
 *********************************************************** */

define(['N/search', 'N/record', 'N/runtime', 'N/file', './datellib'],

    function(search, record, runtime, file,datellib) {
        ...
        function getInputData() {
            ...
            try {
                return search.load({
                    id: 'customsearch_mhl_b2b_attune_test_cancel'
                });
            } catch (e) {
                log.debug({
        ...
        function cancelAndRefund(content, jsonFile, tranDate) {
            try {
                ...
                if (searchData) {
                    ...
                    //var paymentMode=searchData.getText({name:'custbody_mhl_inv_payment_mode'});
                    if (invId) {
                        ...
                        log.debug("Org", Org);
							...
							if(tempArray[te].NSNetAmount){
								n_refundAmount = n_refundAmount + Number(tempArray[te].NSNetAmount);
							}
							else{
                            ...
                            if (SoID) {
								...
								var CanDate = datellib.findDate(CancellationDate, null, subsidiaryTimezone_gmt.custrecord_mhl_timezone_gmt);
                           ...
                           /*      var creditMemo = record.transform({
                                ...
                                isDynamic: false,
                                    ...
                                    for (var te in tempArray) {
                                        n_totalAmount = n_totalAmount + Number(tempArray[te].NetAmount);
									if(tempArray[te].NSNetAmount){
										n_refundAmount = Number(tempArray[te].NSNetAmount);
									}
									else{
									...
									createRnIRecord('Credit Memo is already created ' + vidNumber, jsonFile.name, jsonFile.id,"Credit Memo already Created");
									}
                ...
                //B2C
                    ...
                    if (invId) {
                        ...
                        //if (custType == 2) 
                            ...
                            for (var i in tempArray) {
                                if (i == 0) {
                                    ...
                                    if (valTestCode) {
                                        ...
                                        jsonFile.save();
                                        log.debug("This test code is already cancelled.. ");
                                        createRnIRecord('Json file has test code which is already cancelled in the NetSuite' + vidNumber, jsonFile.name, jsonFile.id,"Test Code Already Cancelled");
                                        return false;
                                    }
                                } else {
                                    formed_testcode_string += tempArray[i].TestCode;
							...
							for (var te in tempArray) {
                                ...
                                jsonFile.folder = '30745';
                                jsonFile.save();
                                createRnIRecord('Credit Memo Not created ' + vidNumber, jsonFile.name, jsonFile.id,"Credit memo not created");
                                return false;
                            }
                    ...
                    if (_validateData(searchResultCount)) {
                        ...
                        createRnIRecord('Json file is fount in ' + fileFolder + ' | ' + vidNumber, jsonFile.name, jsonFile.id,"Json File Found");
                ...
                createRnIRecord(e, jsonFile.name, jsonFile.id,"SuiteScript Error");
                return '';
            }
