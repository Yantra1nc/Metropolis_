/**
 * Script Name:  MHL_YIL_MAP_B2B_Attune_VID_Test_Addition.js
 * @NApiVersion 2.x
 ...
 * Script Name: MHL YIL MAP B2B Attune VID Test Addition
 * File Name: MHL_YIL_MAP_B2B_Attune_VID_Test_Addition.js
 * Created On: 29/05/2022 
 * Modified On:
 * Created By: Avinash Lahane(Yantra Inc.)
 * Modified By:
 * Description: B2B Attune VID Test Addition
 *********************************************************** */
...
define(['N/file', 'N/format', 'N/record', 'N/search','N/runtime','./mhllib'],
	...
	function getInputData() {
		...
		{
			return search.load({
				id:'customsearch_mhl_b2b_attune_test_additio'
			});

		}catch(e)
		{
			createRnIRecord(e,'search Issue',"Test Addition Error");
	...
	function map(context) {
		try {
			...
			var invId=createVIDForJson(content,jsonFile.name,TranDate,fileInternalId);
			...
			if(invId>0)
			{
				jsonFile.folder='30756';
				jsonFile.save();
			}
			else if(invId == '-30757')
			{
				jsonFile.folder='30757';
				jsonFile.save();
			}
		}
		catch (ex) {
			log.error({ title: 'map: error in creating records', details: ex }); 
			createRnIRecord(ex,jsonFile.name,"Error In Map");	
			jsonFile.folder='30757';
			jsonFile.save();
		}
	}
	...
	function createVIDForJson(JSONfromRnI,fileName,TranDate,fileId)
	{
		try
		{
			var flag=0;
					...
					var customerDetails=searchCustomer(JSONfromRnI.VisitInfo.ClientCode,registeredOrg,custSubsidiary,JSONfromRnI.OrgID);
						...
						if(customerSegment && customerSegment!='' && customerSegment!=' ')
						{
							...
							}else
							{
								...
								createRnIRecord('Revenue Item not found for segment '+customerSegment+' For VID '+JSONfromRnI.VisitInfo.VisitNumber,fileName,"Revenue Item Not Found");
								return '-30757';
						...
						}else
						{
							...
							createRnIRecord('Test Missing in VID JSON '+JSONfromRnI.VisitInfo.VisitNumber,fileName,"Test code Missing in JSON");
							return '-30757';
						}
			...
			}else
			{
				...
				if(i_B2B_VID){
					var o_vidObj = record.load({
							...
							var name = o_vidObj.getValue({fieldId: "name"});
							log.debug("name",name);
							var vidNo = o_vidObj.getValue({fieldId: "custrecord_vidno"});
							log.debug("vidNo",vidNo);
							//var vidDate = o_vidObj.getValue({fieldId: "custrecord_vid_date"});
							//log.debug("vidDate",vidDate);
							var vidSbu = o_vidObj.getValue({fieldId: "custrecord_mhl_vid_sbu"});
							log.debug("vidSbu",vidSbu);
							var vidOrg = o_vidObj.getValue({fieldId: "custrecord_mhl_vid_org"});
							log.debug("vidOrg",vidOrg);
							//var vidJsonFile = o_vidObj.getValue({fieldId: "custrecord_mhl_b2b_so_status"});
							//log.debug("vidJsonFile",vidJsonFile);
							
				////////////////////////patient Details////////////////////////////
							var patientEmail = o_vidObj.getValue({fieldId: "custrecord_patient_email_id"});
							log.debug("patientEmail",patientEmail);
							var patientID = o_vidObj.getValue({fieldId: "custrecord_patientid"});
							log.debug("patientID",patientID);
							var patientName = o_vidObj.getValue({fieldId: "custrecord_patientname"});
							log.debug("patientName",patientName);
							var patientNo = o_vidObj.getValue({fieldId: "custrecord_patientnumber"});
							log.debug("patientNo",patientNo);
							var patientSex = o_vidObj.getValue({fieldId: "custrecord_patient_sex"});
							log.debug("patientSex",patientSex);
							var patientAge = o_vidObj.getValue({fieldId: "custrecord_patient_age"});
							log.debug("patientAge",patientAge);
							var patientMobileNo = o_vidObj.getValue({fieldId: "custrecord_patient_mobile_number"});
							log.debug("patientMobileNo",patientMobileNo);
							var Billing_Cycle = o_vidObj.getValue({fieldId: "custrecord_billingcycle"});
							log.debug("Billing_Cycle",Billing_Cycle);
							var coPayClientAmount = o_vidObj.getValue({fieldId: "custrecordcustrecord_copayclientamount"});
							log.debug("coPayClientAmount",coPayClientAmount);
							
									var ConvertedDt = new Date();
							...
							if(soStatus == 'Sales Order:Billed'){
								...
								if(OrderInfoLength.length > 0)
								{
									...
									for (var te in testArray) {
										...
										testObject.testName.push((testArray[te].TestName).toString());
										testObject.testNetAmt.push(testArray[te].Amount);
									   // testObject.testCopAmt.push(testArray[te].Amount);
										testObject.testGross.push(testArray[te].MRP);
										testObject.testDiscount.push(testArray[te].DiscountAmount);
									}
								...
								var o_New_vidObj = record.create({
									type: 'customrecord_b2b_vid_details',
									isDynamic: true

								});

								///* o_New_vidObj.setValue({
...
//}); 
							...
							//need impliment	
								...
								// o_New_vidObj.setValue({
								//	fieldId: 'custrecord_mhl_pdv_ref_doctor',
								//	value: JSONfromRnI.VisitInfo.ReferingDoctor
								//});
								
								//o_New_vidObj.setValue({
								//	fieldId: 'custrecord_mhl_pdv_address',
								...
								{
									for (var te in testGBIArray) {
										if(testGBIArray[te].Code && testGBIArray[te].testName != '')
										{
											a_item_line_data[(testGBIArray[te].Code).toString()] = '';
											testNewGBIArray.testCode.push((testGBIArray[te].Code));
											testNewGBIArray.testName.push((testGBIArray[te].ItemName));
											testNewGBIArray.testNetAmt.push((testGBIArray[te].Amount));
										}
									}

									var testGBIArray = updateInternalId(testNewGBIArray, a_item_line_data);

									log.debug("testGBIArray",JSON.stringify(testGBIArray))

									if (JSONfromRnI.GeneralBillingInfo) {

										for (var g in testGBIArray.testCode) {
											
											if(testGBIArray.testInternalId[g])
											{
												createVID = true;
												var i_item_internal_id = a_item_line_data[testGBIArray.testCode[g]];
												log.debug("testGBIArray i_item_internal_id 579 "+g,i_item_internal_id)
												o_New_vidObj.selectNewLine({
													sublistId: 'recmachcustrecord_reference_b2b'
												});
												o_New_vidObj.setCurrentSublistValue({
													sublistId: 'recmachcustrecord_reference_b2b',
													fieldId: 'custrecord_test_code',
													value: testGBIArray.testInternalId[g]
												});
												o_New_vidObj.setCurrentSublistValue({
													sublistId: 'recmachcustrecord_reference_b2b',
													fieldId: 'custrecord_testname',
													value: testGBIArray.testName[g]
												});
												...
												o_New_vidObj.setCurrentSublistValue({
													...
													value: testGBIArray.testNetAmt[g]
												});
												// 
												//o_New_vidObj.setCurrentSublistValue({
												//	sublistId: 'recmachcustrecord_reference_b2b',
												//	fieldId: 'custrecord_mhl_b2b_test_copayamount',
												//	value: testGBIArray.testNetAmt[g]
											//	}); 
												...
												totalAmount = parseFloat(totalAmount) + parseFloat(testGBIArray.testNetAmt[g]);
												o_New_vidObj.commitLine({
													sublistId: 'recmachcustrecord_reference_b2b'
												});
											}
										}
								...
								for (var testLine in testObject.testCode) {
									var i_item_internal_id = a_item_line_data[testObject.testCode[testLine]];
									log.debug("i_item_internal_id " + testLine, i_item_internal_id)
									if (testObject.testInternalId[testLine] && testObject.testNetAmt[testLine]) {
										
										o_New_vidObj.setValue({
									fieldId: 'externalid',
									value: "vid_" + vidNo+"_"+ testObject.testInternalId[testLine]
								});
								
								o_New_vidObj.setValue({
									fieldId: 'custrecord_vidno',
									value: vidNo +"_"+ testObject.testInternalId[testLine]
								});

								o_New_vidObj.setValue({
									fieldId: 'name',
									value: name +"_"+ testObject.testInternalId[testLine]
								});

										var n_qty = 1;
										var n_price = testObject.testNetAmt[testLine];
										createVID = true;
										//log.debug(testLine,"  testCode "+testObject.testCode[testLine])
										...
										totalAmount = parseFloat(totalAmount) + parseFloat(testObject.testNetAmt[testLine]);
										o_New_vidObj.setCurrentSublistValue({
											sublistId: 'recmachcustrecord_reference_b2b',
											fieldId: 'custrecord_grossamount',
											value: testObject.testGross[testLine]
								...
								if(createVID)
								{
									var o_patientId = o_New_vidObj.save({
										enableSourcing: true,
						...
						{  ////
										...
										for (var te in testArray) {
											...
											if (testArray[te].ProcessingLocationCode) {
												...
												createRnIRecord("Processing Location Code not found for " + JSONfromRnI.VisitInfo.VisitNumber, fileName, fileId,"Processing location Not Found");
												//return '-356151';
											}
				...
				createRnIRecord('VID Number Not found '+JSONfromRnI.VisitInfo.VisitNumber,fileName,"VID Not Found");
				return '-30757';
		...
		}catch(inve)
		{
			...
			createRnIRecord('Error Occured for For VID '+JSONfromRnI.VisitInfo.VisitNumber+' details '+JSON.stringify(inve),fileName,"SuiteScript Error");
			return '-30757';
		}
	}
...
/////////////////////////////////////////////// Create Error Record ////////////////////////////////////////////////////////
	function createRnIRecord(e,fileName,errorTitle)
	{
		...
		if(errorTitle){
		rnIRec.setValue({fieldId: 'custrecord_mhl_rni_inte_errorname',value: errorTitle});
		}
